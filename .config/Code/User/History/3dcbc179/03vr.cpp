...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        cerr << "Используйте: " << argv[0] << " file1 [file2 ...]" << endl;
        return 1;
    }
    ...
    cout << "Общее время выполнения программы: " << total_time << " секунд" << endl;
    cout << "Время слияния файлов: " << merge_time << " секунд" << endl << endl;
    
    cout << "Статистика по корутине:" << endl;
    for (size_t i = 0; i < coroutines_data.size(); ++i) {
        cout << "Файл: " << coroutines_data[i].filename << endl;
        cout << "  Время выполнения: " << coroutines_data[i].duration << " секунд" << endl;
        cout << "  Номер Yield : " << coroutines_data[i].yield_count << endl;
    }
    
    // Вывод первых 20 чисел для проверки
    ...
    // Добавляем запись в файл (новый код)
    ofstream out_file("merged_output.txt");
    if (!out_file) {
    ...
    cout << "Full sorted result saved to: merged_output.txt" << endl;
